            ...
            <div class="bg-white rounded-lg p-6 max-w-sm text-center justify-center items-center flex flex-col gap-5">
                ...
                <div className="text-center">
                    ...
                    <p class="text-gray-500 mb-4">
                        Please visit a hospital or consult your doctor to add or update your health records.
                    </p>
                </div>
            </div>
        </div>
...
const MedicineRecordCard = ({ record }) => {
    return (
      <div
        className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-lg p-6 max-w-sm text-center flex flex-col gap-5 shadow-lg hover:shadow-xl transform transition-all hover:scale-105"
      >
...
const HealthRecords = () => {
    ...
    return (
        <div className="px-5 py-3 bg-gray-100">
            <div className="mb-8">
                <h1 className="text-3xl font-bold text-center mb-4 text-gray-800 py-5">My Health Records</h1>
                <p className="text-lg text-center text-gray-600 mb-12">
                    Keep track of your medical history, including prescriptions, treatments all in one place.
                </p>
            </div>
                        ...
                        <div className="grid grid-cols-1 gap-6">
                            {
                                medicineRecords.map((record, index) => {
                                    return <div key={index} className="bg-white rounded-lg shadow-lg p-6 mb-6">
                                    <p className="text-xl text-gray-700 mb-4 font-bold">
                                        Prescribed on {new Date(record.createdAt).toLocaleString()}
                                    </p>
                                    <ul className="list-number list-inside pl-5 space-y-2">
                                        {record.items && record.items.map((item, index) => (
                                            <li key={index} className="flex flex-col gap-3 bg-emerald-50 p-4 rounded-lg shadow-sm">
                                                <p className="text-lg font-semibold text-gray-800 text-emerald-800">{index+1}. {item.name}</p>
                                                <a
                                                ...
                                                >
                                                    ...
                                                    <span className="text-sm font-medium">Find on BlinkIt</span>
                                                </a>
                                            </li>
                                        ))}
                                    </ul>
